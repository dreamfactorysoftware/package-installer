...
class Installer extends LibraryInstaller implements EventSubscriberInterface
{
	...
	/**
	 * @var array The DSP server configuration
	 */
	protected static $_dspConfig = array();
	...
	public function __construct( IOInterface $io, Composer $composer, $type = 'library', Filesystem $filesystem = null )
	{
		...
		//  Get the system configuration if possible
	...
	protected function _addApplication( PackageInterface $package )
	{
		...
		$_payload = array_merge(
			...
			array(
				...
				'storage_container'   => 'applications/dreamfactory',
				'requires_plugin'     => 1,
				'created_by_id'       => 1,
...
SQL;
		...
		$_data = array(
			':api_name'  => $_apiName = IfSet::get( $_app, 'api-name', $this->_getPackageConfig( $package, '_suffix' ) ),
			':is_active' => 0
		);
		...
		try
		{
			...
			{
				$_message =
					( null === ( $_statement = Sql::getStatement() ) ? 'Unknown database error' : print_r( $_statement->errorInfo(), Verbosity::DEBUG ) );
	...
	{
		$_manifestPath = static::$_platformBasePath . static::DEFAULT_STORAGE_BASE_PATH . $this->_getPackageTypeSubPath( $type ) . static::MANIFEST_PATH_NAME;
	...
	protected function _createLinks( PackageInterface $package )
	{
		...
		foreach ( Option::clean( $_links ) as $_link )
		{
			...
			if ( \is_link( $_linkName ) )
			{
				...
				{
					$this->_log(
						'Link exists but target "<error>' . $_priorTarget . '</error>" is incorrect. Link "' . $_linkName . '" not created.'
					);
				}
		...
		if ( null === ( $_packageConfig = IfSet::get( $_cache, $_packageName = $package->getPrettyName() ) ) )
		{
			...
			if ( null !== ( $_configFile = IfSet::get( $_extra, 'config' ) ) )
			{
				if ( is_string( $_configFile ) && is_file( $_configFile ) && is_readable( $_configFile ) )
				{
					...
					{
						$this->_log(
							'The "config" file specified in this package is invalid: <error>' . $_configFile . '</error>'
						);
...
MYSQL;
		...
		try
		{
			...
			{
				$_message = null === ( $_statement = Sql::getStatement() ) ? 'Unknown database error' : print_r( $_statement->errorInfo(), Verbosity::DEBUG );
